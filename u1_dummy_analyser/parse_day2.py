...
csv = open('day_parsed_less_hours.csv', 'w')

seconds_start = HOUR_START * 3600
seconds_end = HOUR_END * 3600
...
with open(DAY_PATH, 'r') as f:
    ...
    for l in f:
        ...
        if seconds_start <= t <= seconds_end:
            print "saving" + str(t)
            if T.startswith('storage') and msg == 'Request done' and operation in IMPORTANT_OPERATIONS:
                if node_id != '':
                    ...
                    if operation == 'PutContentResponse':
                        ...
                        op = 'mov'
                    print >> csv, str(t - seconds_start) + "," + op + "," + node_id + "," + node_type + "," + mime + "," + str(size) + ","+ str(sid)+ ","+str(user)
