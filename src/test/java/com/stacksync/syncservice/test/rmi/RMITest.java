...
public class RMITest {
	...
	public static void testSetup() throws IOException, SQLException,
		...
		// connection =
		// ConnectionPoolFactory.getConnectionPool(dataSource).getConnection();
		...
		try {
			...
			PostgresqlUserDAO addPostgresqlUserDAO = new PostgresqlUserDAO();
	...
	/*
	 * @Test public void testCreateNewWorkspaceInvalidOwner() {
	 * 
	 * User user = new User(UUID.randomUUID());
	 * 
	 * Workspace workspace = new Workspace(); workspace.setOwner(user);
	 * 
	 * try { workspaceDAO.add(workspace);
	 * assertTrue("User should not have been created", false); } catch
	 * (DAOException e) { assertTrue(e.toString(), true); } }
	 * 
	 * @Test public void testCreateNewWorkspaceValidOwner() throws
	 * IllegalArgumentException, DAOException {
	 * 
	 * User user = new User(); user.setName(nextString());
	 * user.setId(UUID.randomUUID()); user.setEmail(nextString());
	 * user.setQuotaLimit(2048); user.setQuotaUsed(1403); userDao.add(user);
	 * 
	 * Workspace workspace = new Workspace(); workspace.setLatestRevision(0);
	 * workspace.setOwner(user);
	 * 
	 * try { workspaceDAO.add(workspace); assertTrue(true); } catch
	 * (DAOException e) { assertTrue(e.toString(), false); } }
	 * 
	 * @Test public void testCreateObjectInvalidWorkspace() throws
	 * IllegalArgumentException, DAOException {
	 * 
	 * User user = new User(); user.setName(nextString());
	 * user.setId(UUID.randomUUID()); user.setEmail(nextString());
	 ...
	 * 
	 * Item object = new Item(); object.setWorkspace(workspace);
	 * object.setLatestVersion(1L); object.setParent(null);
	 * object.setId(1331432L); object.setFilename(nextString());
	 * object.setMimetype("image/jpeg"); object.setIsFolder(false);
	 * object.setClientParentFileVersion(1L);
	 * 
	 * try { objectDao.put(object);
	 * assertTrue("Object should not have been created", false); } catch
	 * (DAOException e) { assertTrue(e.toString(), true); }
	 * 
	 * }
	 * 
	 * @Test public void testGetObjectByClientFileIdAndWorkspace() throws
	 * DAOException {
	 * 
	 * long fileId = 4852407995043916970L; objectDao.findById(fileId);
	 * 
	 * // TODO Check if the returned obj is correct }
	 * 
	 * @Test public void testGetWorkspaceById() {
	 * 
	 * }
	 * 
	 * @Test public void testGetObjectMetadataByWorkspaceName() throws
	 * DAOException {
	 * 
	 * List<ItemMetadata> objects = objectDao.getItemsByWorkspaceId(UUID
	 * .randomUUID());
	 * 
	 ...
	 * 
	 * @Test public void testGetObjectMetadataByClientFileIdWithoutChunks()
	 * throws DAOException {
	 * 
	 * Long fileId = 538757639L; boolean includeDeleted = false; boolean
	 * includeChunks = false; Long version = 1L; boolean list = true;
	 * 
	 * ItemMetadata object = objectDao.findById(fileId, list, version,
	 * includeDeleted, includeChunks);
	 * 
	 * if (object != null) { System.out.println(object.toString());
	 * 
	 * if (object.getChildren() != null) { for (ItemMetadata child :
	 * object.getChildren()) { System.out.println(child.toString()); } }
	 * assertTrue(true); } else { assertTrue(false); } }
	 * 
	 * @Test public void testGetObjectMetadataByClientFileIdWithChunks() throws
	 * DAOException {
	 * 
	 * Long fileId = 538757639L; boolean includeDeleted = false; boolean
	 * includeChunks = true; Long version = 1L; boolean list = true;
	 * 
	 * ItemMetadata object = objectDao.findById(fileId, list, version,
	 * includeDeleted, includeChunks);
	 * 
	 ...
	 * assertTrue(true); } else { assertTrue(false); } }
	 * 
	 * @Test public void testGetObjectMetadataByServerUserId() throws
	 * DAOException {
	 * 
	 * UUID userId = UUID.randomUUID(); boolean includeDeleted = false;
	 * 
