...
public class PostgresqlItemDAO extends PostgresqlDAO implements ItemDAO {
	...
	@Override
	public Item findById(UUID userID, Long item1ID) throws DAOException {
	...
	public void add(User user, Item item) throws DAOException {
		...
		Object[] values = { user.getId(), item.getWorkspace().getId(), item.getLatestVersion(), item.getParentId(), item.getFilename(),
				item.getMimetype(), item.isFolder(), item.getClientParentFileVersion() };
	...
	public List<ItemMetadata> getItemsByWorkspaceId(UUID userID, UUID workspaceID) throws DAOException {
		...
		String query = "SELECT * FROM get_items_by_workspace_id(?::uuid, ?::uuid)";
	...
	public List<ItemMetadata> getItemsById(UUID userID, Long id) throws DAOException {
		...
		String query = "SELECT * FROM get_items_by_id(?::uuid, ?)";
	...
	public ItemMetadata findById(Long id, Boolean includeList, Long version, Boolean includeDeleted, Boolean includeChunks)
		...
		String query = String.format("WITH    RECURSIVE " + " q AS  " + " (  "
				+ " SELECT i.id AS item_id, i.parent_id, i.client_parent_file_version, " + "     i.filename, iv.version, i.is_folder, "
				+ "     iv.device_id, i.workspace_id, iv.size, iv.status, i.mimetype, " + "     iv.checksum, iv.modified_at, "
				+ "     ARRAY[i.id] AS level_array " + String.format(chunks, "iv") + " FROM    item i "
				+ " INNER JOIN item_version iv ON i.id = iv.item_id AND %s = iv.version " + " WHERE   i.id = ? " + " UNION ALL "
				+ " SELECT i2.id AS item_id, i2.parent_id, i2.client_parent_file_version, "
				+ "     i2.filename, iv2.version, i2.is_folder, "
				+ "     iv2.device_id, i2.workspace_id, iv2.size, iv2.status, i2.mimetype, " + "     iv2.checksum, iv2.modified_at, "
				+ "     q.level_array || i2.id " + String.format(chunks, "iv2") + " FROM    q "
				+ " JOIN    item i2 ON i2.parent_id = q.item_id "
				+ " INNER JOIN item_version iv2 ON i2.id = iv2.item_id AND i2.latest_version = iv2.version "
				+ " WHERE   array_upper(level_array, 1) < ? " + "	) " + " SELECT  array_upper(level_array, 1) as level, q.* "
				+ " FROM    q " + " ORDER BY  " + "       level_array ASC", targetVersion);
	...
	public ItemMetadata findByUserId(UUID userId, Boolean includeDeleted) throws DAOException {
		...
		String query = "WITH RECURSIVE q AS " + " ( " + "    SELECT i.id AS item_id, i.parent_id, i.client_parent_file_version, "
				+ "     i.filename, iv.device_id, i.workspace_id, iv.version, i.is_folder, " + "     iv.size, iv.status, i.mimetype, "
				+ "     iv.checksum, iv.modified_at, " + "     ARRAY[i.id] AS level_array, '/' AS path " + "     FROM user1 u  "
				...
				+ "     JOIN item i2 ON i2.parent_id = q.item_id  "
				+ "     INNER JOIN item_version iv2 ON i2.id = iv2.item_id AND i2.latest_version = iv2.version  "
				+ "     WHERE array_upper(level_array, 1) < 1 " + " )  " + " SELECT array_upper(level_array, 1) as level, q.*  "
	...
	public List<String> migrateItem(Long itemId, UUID workspaceId) throws DAOException {
		...
		String query = "WITH    RECURSIVE " + " q AS " + " ( " + " SELECT i.* " + " FROM    item i " + " WHERE   i.id = ? " + " UNION ALL "
				+ " SELECT i2.* " + " FROM    q " + " JOIN    item i2 ON i2.parent_id = q.id " + " ) "
				+ " UPDATE item i3 SET workspace_id = ?::uuid " + " FROM q " + " WHERE q.id = i3.id";
