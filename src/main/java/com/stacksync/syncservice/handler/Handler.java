...
public class Handler {
	...
	public List<CommitInfo> doCommit(User user, Workspace workspace, Device device, List<ItemMetadata> items) throws DAOException {
		...
		workspace = workspaceDAO.getById(user.getId(), workspace.getId());
		// TODO: check if the workspace belongs to the user or its been given
		// access

		device = deviceDao.get(user.getId(), device.getId());
		...
		for (ItemMetadata item : items) {
			...
			try {
				...
				objectResponse = this.getCurrentServerVersion(user, serverObject);
				committed = false;
	...
	private ItemMetadata getCurrentServerVersion(User user, Item serverObject) throws DAOException {
		return getServerObjectVersion(user, serverObject, serverObject.getLatestVersion());
	}
